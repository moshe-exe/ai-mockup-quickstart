...
// ────────────────────────────────────────────────────────────────────
// CHANGE ME #1 — Schema + matching TypeScript shape.
// ────────────────────────────────────────────────────────────────────
type Item = {
  id: string;
  title: string;
...
function validate(item: any): string[] {
  const errors: string[] = [];
...
Return ONLY a JSON object with an "items" array of ${COUNT} entries.`;
  ...
  const items: Item[] = Array.isArray(parsed) ? parsed : parsed.items ?? [];

  const valid: Item[] = [];
  const bad: { item: any; violations: string[] }[] = [];
  for (const item of items) {
    const violations = validate(item);
    if (violations.length) bad.push({ item, violations });
    else valid.push(item);
  }

  writeFileSync(OUTPUT_PATH, JSON.stringify(valid, null, 2));
